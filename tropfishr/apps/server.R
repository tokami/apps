...
source("../functions/LCCC_fun.R")
source("../functions/natM_fun.R")
##source("../functions/VPA_fun.R")
source("../functions/YPR_fun.R")
...
shinyServer(
    function(input, output, session){
        ...
        output$image <- renderImage({
            return(list(
                src = "images/zanzibarFoto2.JPG",
                contentType = "image/jpeg",
                width = "100%",
                alt = "Fisheries on Zanzibar",
                caption = "Fisheries on Zanzibar"
            ))
        }, deleteFile = FALSE)
       ...
       output$downloadExData <- downloadHandler(
         ...
         content = function(con) {
           ...
           res$frequency <- sapply(seq(nrow(res)), function(i) dat[, -1][dat[, 1] == res[i, 1], names(dat)[-1] == res[i, 2]])
        ...
        output$contents <- renderTable({
            if(rv$doDatLoad == FALSE){
                ...
                if(!input$useExDat && !"length" %in% colnames(rv$dat)){
                    stop("Something went wrong when uploading the data! Please make sure that you have a column with the length measurements in your file! This column must be called 'length'")
                    return()
                }
                if(!input$useExDat && !"dates" %in% colnames(rv$dat)){
                    stop("Something went wrong when uploading the data! Please make sure that you have a column with the sampling dates in your file! This column must be called 'dates'!")
                    return()
                }                                
                datPars()
                dat <- rv$dat
...
##         })        
        ...
        LBSPR_res <- function(){
            ...
            lfq <- lfqModify(lfq, vectorise_catch = TRUE,
                             bin_size = input$binSizeYPR)            
            lbsprPars <- new("LB_pars")
            lbsprPars@Linf <- rv$parsGrowth$Linf
            lbsprPars@MK <- rv$resnatM/rv$parsGrowth$K
            lbsprPars@L_units <- "cm"
            lbsprPars@L50 <- as.numeric(input$Lmat)
            lbsprPars@L95 <- as.numeric(input$Lmat) * 1.3 ## definition  ### change!
            lbsprPars@Walpha <- as.numeric(input$LWa)
            lbsprPars@Wbeta <- as.numeric(input$LWb)
            lbsprPars@BinWidth <- input$binSizeYPR
            
            lbspr <- new("LB_lengths", LB_pars = lbsprPars)

            ## maxL must be larger than Linf
            if(max(lfq$midLengths) < lfq$par$Linf){
                lbspr@LMids <- seq(min(lfq$midLengths), lfq$par$Linf+input$binSizeYPR,
                                   input$binSizeYPR)
                tmp <- rbind(as.matrix(lfq$catch),matrix(0, ncol=ncol(as.matrix(lfq$catch)),
                                            nrow=(length(lbspr@LMids) - length(lfq$midLengths))))
                                     
                lbspr@LData <- tmp
            }else{
                lbspr@LMids <- lfq$midLengths
                lbspr@LData <- as.matrix(lfq$catch)
            }
            lbspr@L_units <- "cm"
            lbspr@Years <- 1
            lbspr@NYears <- 1
            
            resLBSPR <- LBSPRfit(LB_pars = lbsprPars,
                              LB_lengths = lbspr,
                              yrs = 1,
                              Control = list(modtype = "GTG")) ## all functions in TryCatch() ?


            rv$lbspr <- lbspr
            rv$lbsprPars <- lbsprPars            
            
            lbsprParsTar <- lbsprPars
            lbsprParsTar@SPR <- input$targetSPR                ## Target SPR             
            resLBSPRTar <- LBSPRfit(LB_pars = lbsprParsTar,
                              LB_lengths = lbspr,
                              yrs = 1,
                              verbose=FALSE)
            
            yr <- 1 # first year of data
            lbsprParsTar@SL50 <- resLBSPRTar@SL50[yr]
            lbsprParsTar@SL95 <- resLBSPRTar@SL95[yr]

            rv$lbsprParsTar <- lbsprParsTar

            rv$resLBSPR <- resLBSPR
            pars <- resLBSPR@Ests
##            names(pars) <- ll
            rv$parsLBSPR <- pars            
        }
        
        output$lbsprPars <- renderTable({
            if(rv$doYPR == FALSE){
                return()
            }else{
                isolate({
                ...
                tmp <- as.data.frame(rv$parsLBSPR)
                tmp
            }
        })


        output$LBSPR1_plot <- output$LBSPR1_plot_ov <- renderPlot({
            if(rv$doYPR == FALSE)
                return()
                par(mar=c(5,5,2,1))
                plotSize(rv$resLBSPR)                
        })

        output$LBSPR2_plot <- output$LBSPR2_plot_ov <- renderPlot({
            if(rv$doYPR == FALSE)
                return()
                par(mar=c(5,5,2,1))
                plotMat(rv$resLBSPR)                
        ...
        output$LBSPR3_plot <- output$LBSPR3_plot_ov <- renderPlot({
            if(rv$doYPR == FALSE)
                return()
                par(mar=c(5,5,2,1))
                plotEsts(rv$resLBSPR)                
        })

        output$LBSPR4_plot <- output$LBSPR4_plot_ov <- renderPlot({
            if(rv$doYPR == FALSE)
                return()
                par(mar=c(5,5,2,1))
                plotTarg(rv$lbsprParsTar, rv$lbspr, yr=1)  
        })        
        ##-----------------------------------------------------------                
        ...
        output$lbsprPars_ov <- renderTable({
            if(rv$doYPR == FALSE)
                return()
                isolate({            
                tmp <- as.data.frame(rv$parsLBSPR)
        ...
        output$allParameters <- downloadHandler(
            filename = function(){
                filename = strsplit(rv$filename,"Data: ")[[1]][2]
                paste0("STF_allpars_",filename,"_",Sys.Date(),".csv")
            },
            content = function(con){
        ...
        output$allGraphs <- downloadHandler(
            filename = function(){
                filename = strsplit(rv$filename,"Data: ")[[1]][2]
                paste0("STF_allgraphs_",filename,"_",Sys.Date(),".zip")
            },
            content = function(con){
                graphs <- c("lfq","ELEFANfit","ELEFANscore",
                            "cohorts","recruitment","catchcurve",
                            "ypr","LBSPR_lfq", "LBSPR_mat","LBSPR_est",
                            "LBSPR_target")
                fs <- c()
                tmpdir <- tempdir()
                ##                setwd(tempdir())
        ...
        output$allData <- downloadHandler(
            filename = function(){
                filename = strsplit(rv$filename,"Data: ")[[1]][2]
                paste0("STF_alldata_",filename,"_",Sys.Date(),".RData")
            },
            content = function(con){
                save(rv, file = con)
            }
